import numpy as np
from vertices import Input, Dot, Softmax, CrossEntropy
...
if __name__ == "__main__":
    ...
    w.value = np.random.normal(0, 0.001, (3, 2))

    dot = Dot([x, w], name="dot")
    p = Softmax([dot], name="softmax")
    ...
    x_full = np.hstack([x_full, np.ones(N).reshape(N, 1)])
